...
class MyVisitor(CSVVisitor):
    def __init__(self):
        self.fields_counter = 0  # Contador de campos del CSV
        self.headers = []  # Lista con el encabezado
        self.filas = []

    def visitFile(self, ctx: CSVParser.FileContext):
        if ctx.header():
            self.headers = self.visit(ctx.header())
        for row in ctx.row():
            rows = self.visit(row)
            # Conversión de campos string -> float
            r2, r3, r4, r5, r6, r7, r8, r9 = map(float, (
                rows[2], rows[3], rows[4], rows[5], rows[6], rows[7], rows[8], rows[9]))
